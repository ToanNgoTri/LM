 ...
 *
 * Module sống trong chính app (android/app/.../speech, ios/lawMachine/
...
export function addListeners({
  ...
  const pairs = [
    ...
    ['onSpeechPartial', onPartial],
    ['onSpeechFinal', onFinal],
    ['onSpeechVolume', onVolume],
    ['onSpeechEnd', onEnd],
    ['onSpeechError', onError],
  ];

  // Gọi emitter NHƯ MỘT PHƯƠNG THỨC của Native (Native[key](handler)). Tách nó
  // ra biến rồi gọi trần sẽ mất receiver.
  const subscriptions = pairs
    .filter(([key, handler]) => Native[key] && handler)
    .map(([key, handler]) => Native[key](handler));
